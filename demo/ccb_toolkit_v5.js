    ...
    function Parser(require, module, exports) {
        ...
        Parser.prototype = {
                ...
                var instance = this,
                    initMethod = normailzeSurrogate(function (method, name) {
                        ...
                        return function () {
                            ...
                            /**
                             * 检查一次该方法是否被模板依赖，若是，则获取该方法的依赖，当依赖变更时，使模板变为 rerender 状态。
                             */
            ...
            _initRootContext: function () {
                function RootContext() { }// 根上下文,存放各种不会变的引用
                function DataContext() { }// 数据上下文,存放 data, props, computed 的数据
                function Context() { } // 作用域上下文,存放 for 指令的别名数据
            ...
            _initDataContext: function (config) {
                ...
                for (var key in data) {
                    ...
                    if (props && props.hasOwnProperty(key)) {
                        this._logError("data 和 props 对象中存在重复定义的属性：" + key)
                    }
                    else {
            ...
            _initComputed: function (userDefineComputed) {
                ...
                for (var name in userDefineComputed) {
                    var errorMsg = 0
                        || Reg$NestingPath.test(name) && "不要使用键路径的形式命名"
                        || dataContext.hasOwnProperty(name) && "与 data 或 props 中的属性冲突"
                        || !isFunction(userDefineComputed[name]) && "必须使用函数定义"

                    if (errorMsg) {
                        throw this._logError("computed 的【" + name + "】" + errorMsg)
                    }
                    else {
            ...
            _setData: function (keyPath, val, force) {
                ...
                for (var keyPath$ in dataPatch) {
                    if (!propsConfig || !(keyPath$ in propsConfig)) {
                        ...
                        this._logError("不能使用 $setData 修改 props 数据：" + keyPath$)
                    }
                }
            ...
            _createConvertor: function () {
                ...
                function dataOwns(dataKey) {
                    if (
                        ...
                        throwError("model 指令必须使用 data 选项里声明的状态，接收到【" + dataKey + "】")
                    }
                }
                ...
                function processRef(token) {
                    ...
                    if (uniq[binding.value]) return instance._logWarning("ref 指令使用的名称重复：" + binding.value)
                            ...
                            if (hasComponentChildren) {
                                ...
                                try {
                                    ...
                                    instance._logError([
                                        "渲染插槽时出错",
                                        ["错误信息：", e],
                                        ["错误位置 - 第" + (i + 1) + "个组件子元素", getRawFn(componentSlotChildren[i])]
                                    ])
                                }
                            }
                            else {
        ...
        var parseTemplate = function () {
            var Reg$ElemOpenStart = /^<([\w\u4e00-\u9fa5-]+)\s*/,
                ...
                elemAttrValue = // 在mustache、圆括号内可直接使用与属性值引号相同的引号
                    '(["\'])' + // 起始属性值引号
                    // 属性值内容包含这些情况：一般字符 | mustache | 圆括号 | 与属性值引号不同的引号 | 带转义符的属性值引号 | 不匹配mustache,parenthesis的{(
